<?php
function onRedis($key, $value, $time) {
    $redis = new Redis();
...
if ($sig != md5($str.$secret_key)) {
  ...
  switch ($input['notification_type']) {
    ...
    $item_tranzaktion  = intval($item[2]);

    $user_id = $input['user_id'];

    onRedis('abt_'.$user_id ,$item_tranzaktion,300);
...
if ($item_name == 'cr') {
        if ($item_num>0) {
            $response['response'] = array(
                ...
                'photo_url' => 'http://tanks.github.com/vk_frame/credits_icon.jpg',
                'price' => $item_num
            );
        } else {
            $response['error'] = array(
